      ...
      <div class="row">
        ...
        <div class="col-sm-12 col-md-12 col-lg-12">
          ...
          <ol class="breadcrumb breadcrumb1">
            ...
            <li class="active">{{ __('main.low-price') }} </li>
          </ol>
          ...
          <div class="all-products">
            <div class="">
              <h2 class="title-div wow slideInRight" data-wow-duration="1s" data-wow-delay="0s" data-wow-offset="10">
                {{ __('main.products') }} {{ __('main.low-price') }} </h2>
              <div class="products">
                <div class="row">

                  @foreach ($lowPriceProducts as $lowPriceProduct)

                    <div class="col-md-3">
                      <div class="product-item">
                        <div class="product-borde-inner">
                          <a href="{{ url("/products/show/{$lowPriceProduct->id}") }}">
                            <img src="{{ asset("uploads/$lowPriceProduct->img") }}" class="img img-responsive" />
                          </a>


                          <div class="product-price">
                            <a href="{{ url("/products/show/{$lowPriceProduct->id}") }}">
                              {{ $lowPriceProduct->name() }}</a><br />


                            <span class="prev-price">
                              <del>{{ $lowPriceProduct->price }}$</del>
                            </span>
                            <span class="current-price">
                              @php
                                $discount = ($lowPriceProduct->price * $lowPriceProduct->discount) / 100;
                                $newPrice = $lowPriceProduct->price - $discount;
                              @endphp
                              {{ $newPrice }}$
                            </span>
                          </div>

                          @if ($lowPriceProduct->quantity > 0)
                          <form id="addToCart" action="{{url("/cart/add-cart/{$lowPriceProduct->id}") }}" method="post" style="display: block;">
                            @csrf
